(function() {
  var en, extend, lingo, package, slug, word;
  ...
  word.pluralize = function(count, singular, plural) {
    if (plural == null) plural = null;
    if (count === 1) return singular;
    if (!plural) plural = en.pluralize(singular);
    return plural;
  };

  word.join = function(words, last) {
    if (Array.isArray(words) === false) words = words.split(/\s+/g);
    return lingo.join(words, last);
  };

  word.slug = function(str) {
    return slug(str);
  };
